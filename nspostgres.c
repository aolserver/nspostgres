 ...
 * Ns_PgDbType --
 *
 *     Return the string which identifies the PostgreSQL database.
 *
 * Results:
 *
 * Side effects:
 ...
 * Ns_PgOpenDb --
 *
 *     Open an PostgreSQL connection on an nsdb handle. The
 ...
 * Ns_PgCloseDb --
 *
 *     Close an PostgreSQL connection on an nsdb handle.
 *
 * Results:
 *
 * Side effects:
 ...
 *
 *     Set the current transaction state based on the query pointed to
 *     by"sql".  Should be called only after the query has
 ...
 * Ns_PgExec --
 *
 *     Send a PostgreSQL query.  This function does not implement an
 *     nsdb function but is used internally by the PostgreSQL driver.
 *
 * Results:
 *
 * Side effects:
 ...
 * Ns_PgSelect --
 *
 *     Send a query which should return rows.
 *
 * Results:
 *
 * Side effects:
 ...
 * Ns_PgGetRow --
 *
 *     Fetch rows after an Ns_PgSelect or Ns_PgExec.
 *
 * Results:
 *
 * Side effects:
 ...
 * Ns_DbTableInfo --
 *
 *     Return system catalog information (columns, types, etc.)
 *     about a table.
 *
 * Results:
 *
 ...
 * Ns_PgBestRowId --
 *
 *     Return the primary key of a table.  If a table has a primary
 *     key, AOLserver can perform row updates and deletes.  In the
 *     case of PostgreSQL, the "oid" system column is alwasy unique
 *     so we just return it instead of looking for an actual primary
 *     key.
 *
 * Results:
 *
 ...
 *     that apostrophe isn't legal in Postgres strings. The
 *     uuencoding algorithm doesn't make use of lower case letters,
 ...
 *
 * Write the contents of BUFP to a file descriptor or to the network
 * connection directly. Lifted from Oracle driver.
 *
 * Results:
 *
 * Side effects:
 ...
 *
 *     Parse a SQL string and return a list of all the bind
 *     variables found in it.
 *
 * Results:
 *
 * Side effects:
 ...
 * PgBindCmd --
 *
 *     This function implements the "ns_pg_bind" Tcl command 
 *     installed into each interpreter of each virtual server. It
 *     provides for the parsing and substitution of bind variables
 *     into the original sql query.  This is an emulation
 *     only. Postgresql doesn't currently support true bind
 *     variables yet.
 *
 * Results:
 *
 ...
 * Ns_PgCmd --
 *
 *     This function implements the "ns_pg" Tcl command installed into
 *     each interpreter of each virtual server.  It provides access
 *     to features specific to the PostgreSQL driver.
 *
 * Results:
 *
 * Side effects:
 ...
 * Ns_PgInterpInit --
 *
 *     Add the "ns_pg" command to a single Tcl interpreter.
 *
 * Results:
 *
 * Side effects:
 ...
 * Ns_PgServerInit --
 *
 *     Have Ns_PgInterpInit called for each interpreter in the virtual
 ...
 * Ns_PgBindRow --
 *
 *    Retrieve the list of column names of rows.
 * 
 * Results:
 *
 * Side effects:
 ...
 *     to the normal operation of the ACS (ArsDigita Community
 *     System).  We include definitions for them here
 *
 * Results:
 *
 * Side effects:
		   ...
		   int argc, char *argv[])
{
	...
	if (pg_get_column_index (interp, tinfo, argv[4], &colindex) != TCL_OK) {
	    goto bailout;
	}
